...
for (let i = 0, len = iScrollEventNames.length; i < len; i++) {
  ...
  const reactEventName = `on${iScrollEventName[0].toUpperCase()}${iScrollEventName.slice(
    1
  )}`;
...
export default class ReactIScroll extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextContext) {
    return (
      !isEqual(this.props, nextProps) || !isEqual(this.context, nextContext)
    );
  }
  ...
  getIScrollInstance() {
    console.warn(
      "Function 'getIScrollInstance' is deprecated. Instead use 'getIScroll'"
    );
    return this._iScrollInstance;
  }
    ...
    if (prevPropEvent !== currentPropEvent) {
      ...
      this.withIScroll(true, function(iScrollInstance) {
        if (typeof prevPropEvent === "function") {
          iScrollInstance.off(
            iScrollEventName,
            currentEvents[iScrollEventName]
          );
